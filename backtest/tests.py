...
class LayeredStrategyEngineTests(SimpleTestCase):
    ...
    def make_indicators(self, count, regimes=None, rsi=None, ma20=None, ma60=None):
    ...
    def test_bull_or_neutral_trend_opens_core_on_next_open(self):
        ...
        self.assertEqual(core_buy['reason'], 'CORE_TREND_ENTRY')
    ...
    def test_swing_risk_and_atr_parameters_change_only_swing_sizing(self):
        ...
        indicators = self.make_indicators(
            len(bars),
            rsi=[Decimal('50'), Decimal('35'), Decimal('46'), Decimal('50'), Decimal('50'), Decimal('50'), Decimal('50')],
        )
        ...
        wider_atr = self.run_engine(
            bars=bars,
            indicators=indicators,
            parameters=StrategyParameters(swing_atr_multiplier=Decimal('2.4')),
        )
        ...
        self.assertEqual(lower_swing, baseline_swing / Decimal('2'))
        self.assertEqual(wider_swing, baseline_swing / Decimal('2'))
    ...
    def test_swing_exit_does_not_reduce_core_and_counts_one_cycle(self):
        ...
        indicators = self.make_indicators(
            len(bars),
            rsi=[Decimal('50'), Decimal('35'), Decimal('46'), Decimal('65'), Decimal('50'), Decimal('50'), Decimal('50')],
        )

        result = self.run_engine(bars=bars, indicators=indicators)
        swing_sell = next(trade for trade in result['trades'] if trade['position_layer'] == 'SWING' and trade['type'] == 'SELL')

        self.assertEqual(swing_sell['reason'], 'SWING_RSI_EXIT')
    ...
    def test_core_exit_forces_swing_exit_without_mixing_layers(self):
        ...
        self.assertEqual(sells[0]['reason'], 'SWING_FORCED_EXIT')
        self.assertEqual(sells[1]['reason'], 'CORE_TREND_EXIT')
    ...
    def test_persistent_conditions_do_not_repeat_swing_entry(self):
        ...
        indicators = self.make_indicators(
            len(bars),
            rsi=[Decimal('50'), Decimal('35'), Decimal('46'), Decimal('35'), Decimal('46'), Decimal('35'), Decimal('46'), Decimal('50'), Decimal('50')],
        )
    ...
    def test_same_core_position_allows_repeated_swing_cycles_after_cooldown(self):
        bars = self.make_bars(count=11)
        indicators = self.make_indicators(
            len(bars),
            rsi=[
                Decimal('50'), Decimal('35'), Decimal('46'), Decimal('60'), Decimal('50'),
                Decimal('35'), Decimal('46'), Decimal('60'), Decimal('50'), Decimal('50'), Decimal('50'),
            ],
        )
        ...
        self.assertEqual(result['average_days_per_swing_cycle'], Decimal('1'))
    ...
    def test_swing_diagnostics_explain_why_entries_are_blocked(self):
        ...
        indicators = self.make_indicators(
            ...
            rsi=[Decimal('50'), Decimal('35'), Decimal('46'), Decimal('50'), Decimal('50'), Decimal('50'), Decimal('50'), Decimal('50')],
        )
    ...
    def test_swing_cooldown_blocks_an_early_second_cycle(self):
        bars = self.make_bars(count=11)
        indicators = self.make_indicators(
            len(bars),
            rsi=[
        ...
        result = self.run_engine(
            bars=bars,
            indicators=indicators,
            parameters=StrategyParameters(swing_cooldown_days=3),
        )

        self.assertEqual(result['swing_cycle_count'], 1)
        self.assertEqual(result['swing_entry_count'], 1)
        self.assertEqual(result['swing_exit_count'], 1)

    def test_configurable_swing_rsi_levels_control_entry_and_exit(self):
        bars = self.make_bars(count=8)
        indicators = self.make_indicators(
            len(bars),
            rsi=[Decimal('55'), Decimal('45'), Decimal('51'), Decimal('58'), Decimal('55'), Decimal('55'), Decimal('55'), Decimal('55')],
        )
        ...
        self.assertEqual(swing_trades[1]['signal_date'], bars[3].date.isoformat())
...
class MarketRegimeBacktestApiTests(APITestCase):
    ...
    def test_real_daily_prices_drive_hybrid_and_all_three_comparisons(self):
        ...
        self.assertEqual(response.data['strategy_parameters']['swing_cooldown_days'], 2)
        ...
        first_point = response.data['equity_curve'][0]
        self.assertEqual(first_point['date'], self.start_date.isoformat())
        for field in ('ma20', 'ma60', 'rsi14', 'atr14', 'bollinger_upper', 'market_regime'):
    ...
    def test_each_selected_symbol_fills_its_own_warmup_history(self, fetch_prices):
        ...
        def fill_history(security, first_date, end_date):
