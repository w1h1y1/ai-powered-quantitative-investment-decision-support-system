"""AG-02: DeepSeek reads agent_context_v1 and returns structured analysis."""
...
def _unavailable(context, provider, model, reason):
    logger.warning(
        'Agent analysis unavailable provider=%s model=%s symbol=%s reason=%s',
        provider,
        model,
        context.get('symbol'),
        reason,
...
def _normalize_result(result, context):
    ...
    if not isinstance(result.analysis, dict):
        return None
    try:
        return normalize_unified_analysis(result.analysis, context)
    except UnifiedAnalysisValidationError:
        return None
...
def run_agent_analysis(security, user, *, provider=None):
    ...
    if not llm_provider.is_configured():
        return _unavailable(context, provider_name, model, 'LLM service is not configured.')
    ...
    analysis = _normalize_result(result, context)
